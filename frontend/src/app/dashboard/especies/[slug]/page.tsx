                ...
                {trait.options.map((option) => (
                  <div key={option.id} className="flex justify-between items-start p-3 border border-stone-100 rounded hover:border-stone-300 transition-colors">
                    ...
                    <div className="flex gap-1 ml-2">
                      {/* Editamos pasando el objeto 'option' pero casteado a DnDTrait, ya que comparten estructura base */}
                      <button onClick={() => handleOpenEdit(option as any)} className="p-1.5 text-stone-400 hover:text-stone-700">
                        <FaPencilAlt className="w-3 h-3"/>
                      </button>
                      <button onClick={() => handleDeleteClick(option as any)} className="p-1.5 text-red-300 hover:text-red-600">
