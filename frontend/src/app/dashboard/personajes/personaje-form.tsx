...
export default function PersonajeForm({ onSave, onCancel, initialData }: PersonajeFormProps) {
  ...
  // Función para calcular nivel automáticamente en el formulario (visual)
  const calculateLevel = (cp: number) => {
    let level = 1;
    let remaining = cp;

    while (remaining > 0 && level < 20) {
      let cost = 4;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => {
        ...
        // Si cambiamos checkpoints, calculamos el nivel automáticamente
        if (name === 'checkpoints' && typeof val === 'number') {
            newData.nivel = calculateLevel(val);
        }
        return newData;
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 font-body text-stone-800 max-h-[80vh] overflow-y-auto pr-4 scrollbar-custom">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Reemplazamos el input de Nivel manual por Checkpoints y Nivel calculado */}
        <div>
            <label className="block mb-1 font-semibold">Checkpoints</label>
            <Input name="checkpoints" type="number" value={String(formData.checkpoints)} onChange={handleChange} />
        </div>
        <div>
            <label className="block mb-1 font-semibold">Nivel (Calc.)</label>
            <Input 
                name="nivel" 
                type="number" 
                value={String(formData.nivel)} 
                onChange={handleChange} 
                disabled // Deshabilitado porque se calcula solo
