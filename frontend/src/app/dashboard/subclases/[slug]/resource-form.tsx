...
import { SubclassResource } from '@/types';
import Input from '@/components/input';
import Button from '@/components/button';
import Dropdown from '@/components/dropdown';

interface ResourceFormProps {
  onSave: (data: Partial<SubclassResource>) => void;
  onCancel: () => void;
  initialData?: SubclassResource | null;
  subclassId: number;
}
...
export default function ResourceForm({ onSave, onCancel, initialData, subclassId }: ResourceFormProps) {
  const [formData, setFormData] = useState<Partial<SubclassResource>>({
    name: '',
    reset_on: 'Short Rest',
    quantity_type: 'Fixed',
    quantity_stat: '',
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ 
          name: '', reset_on: 'Short Rest', quantity_type: 'Fixed', 
          quantity_stat: '', progression: {}, value_progression: {} 
      });
      setQtyLevels({});
      setValLevels({});
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    } as unknown as Partial<SubclassResource>);
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6 font-body text-stone-800">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="col-span-2">
            ...
            <Input name="name" value={formData.name} onChange={handleChange} required placeholder="Ej: Superiority Dice" />
        </div>
        <div>
        ...
        {formData.quantity_type === 'Stat' && (
             <div>
                ...
                <Dropdown name="quantity_stat" value={formData.quantity_stat} onChange={handleChange} options={STAT_OPTIONS} />
            </div>
        )}
        ...
        <div className="overflow-x-auto border border-stone-300 rounded-lg">
            <div className="min-w-[800px] grid grid-cols-[40px_repeat(20,1fr)] text-xs">
                ...
                {Array.from({length: 20}, (_, i) => i + 1).map(lvl => (
                    <div key={lvl} className="bg-stone-100 p-1 text-center border-b border-stone-200 font-semibold text-stone-500">{lvl}</div>
                ))}

                {formData.quantity_type === 'Fixed' && (
                    <>
                        <div className="p-2 font-bold text-stone-600 border-r border-stone-200 bg-stone-50 flex items-center justify-center writing-mode-vertical">Cant.</div>
                        {Array.from({length: 20}, (_, i) => i + 1).map(lvl => (
                ...
                <div className="p-2 font-bold text-stone-600 border-r border-stone-200 bg-stone-50 flex items-center justify-center">Valor</div>
                {Array.from({length: 20}, (_, i) => i + 1).map(lvl => (
